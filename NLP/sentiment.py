...
maxlen = 128
batch_size = 32
bert_Path = r"Models\Bert_EN_Uncased_L-12_H-768_A-12_Google-Research"
config_path = bert_Path + r"\bert_config.json"
checkpoint_path = bert_Path + r"\bert_model.ckpt"
...
model.compile(loss='sparse_categorical_crossentropy', # optimizer=Adam(1e-5),  # 用足够小的学习率
...
def load_data():
    ...
    D = [(reviews, int(labels)) for reviews, labels in zip(data["review"], data["label"])]
    
    iloc = int(len(D) * 0.8)
    train = D[:iloc]
    test = D[iloc:iloc+int((len(D)-iloc)/2)]
    valid = D[iloc+int((len(D)-iloc)/2):-1]
